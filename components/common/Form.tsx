    ...
    >
      {title && (
        <h2
          ...
          } text-gray-200 text-2xl font-bold text-center`}
        >
          {title}
        </h2>
      )}
            ...
            {inputs.map(({ type, label, name, placeholder }, index) => (
              <div key={`item-input-${index}`} className="mx-0 mb-1 sm:mb-4">
                ...
                <input
                  ...
                  className="mb-2 w-full rounded-md border border-gray-400 py-2 pl-2 pr-4 shadow-md text-gray-300 bg-gray-700 sm:mb-0"
                />
              </div>
            ))}
          </div>
