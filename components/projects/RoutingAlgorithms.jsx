...
const code =
  "private static void swapNodes(int pos1, int pos2) { // first modify the position array positionArray[vertexArray[pos1]] = pos2; positionArray[vertexArray[pos2]] = pos1; // then we actually switch the vertex Array int tempVertices = vertexArray[pos1];}";
    ...
    >
          ...
          <div className="w-full space-y-4 sm:text-base text-sm ">
            <div className="flex flex-col font-Header ">
              ...
              <span className="text-gray-400  tracking-wide">
                Network optimization is a significant topic of current computer
                science and computer engineering study. In this course project,
                I have created a network routing protocol utilizing the data
                structures and algorithms learned in class CSE629 Analysis of
                Algorithms. This allowed me to transform my academic
            ...
            <div className="font-Header tracking-wide">
              ...
              <span className="text-gray-400 mb-4">
                The goal of this project is to help us realize that translating
                algorithmic notions at a higher degree of abstraction into
                tangible implementations in a specific programming language is
                not always straightforward. Implementations usually urge you to
                focus on extra details of the techniques, which can result in
                significantly more knowledge. We concentrate on the Maximum
                Bandwidth Path problem, which is considered a significant
                network optimization challenge. Various techniques may be used
                to find the path with the best potential bandwidth between a
                given source and destination, but the performance is mainly
                dependent on the actual implementation of an algorithm and the
                data structures utilized beneath. First, we build two random
                weighted undirected graphs, one sparse and one dense with a
                large degree of vertices. We maintain this generation
                unpredictable enough to adequately assess the performance of
                methods. Furthermore, we employ two well-known algorithms -
                Dijkstra’s and Kruskal’s - to determine the maximum bandwidth
                path between the source and destination. However, we conduct a
                comprehensive examination of the running time of these
                algorithms after modifying them to use the heap sort rather than
                not utilizing one. In the later parts of this report, I go over
                the specifics of random graph generation, implementation of
                Dijkstra’s without heap algorithm, Dijkstra’s with heap
                algorithm, and Kruskal’s algorithm. Finally, I present the
                results in tabular format as well as a thorough performance
                comparison of these three routing algorithms.
              </span>
            </div>
            ...
            <div className="flex flex-col font-Header tracking-wide">
              ...
              <span className="text-gray-400 mb-4">
                I build these random graphs in two parts. Firstly, each vertex
                is connected to its next vertex thus forming a cycle to ensure
                that graph has only one strongly connected component. For
              ...
              >
                <MathComponent
                  tex={String.raw`\text{Average Degree} = \frac{2 \cdot \text{Number of edges}}{\text{Number of vertices}}`}
                />
              </span>
              <span className="text-gray-400 mb-4">
                Because both graphs are weighted, when I create an edge between
                any two vertices, I apply a random edge weight (generated by the
                Random library from the java.util package) with a weight limit
                of 20000. I tried numerous settings for this weight limit
                option, rang- ing from 10 to 50,000. Because the Random library
                was producing a random integer (not a float), the number of
                buckets for the dense network was relatively smaller for the
                lower weight limit, resulting in saturating the maximum
            ...
            <div className="flex flex-col font-Header tracking-wide">
              ...
              <span className="text-gray-400 mb-4">
                We are required to implement the maximum heap subroutine, which
                performs fundamental heap operations such as inserting,
                  ...
                  <li>
                    ...
                    <span>
                      <CodeBlock
                        ...
                        text={`private static void swapNodes(int pos1, int pos2) { 
      ...
      heapArray[pos2] = dummy;
                  ...
                  <li>
                    ...
                    >
                      <MathComponent
                        tex={String.raw`index \in \left( size , \frac{size}{2}\right]`}
                      />
                    </span>
                  </li>
                </ul>
